...
export const StatsHUD: React.FC = () => {
  const { lastJsonMessage, gameState, sessionData } = usePuttingState();
  ...
  return (
    <>
      {/* Main Shot Stats HUD (Top Center) */}
      <AnimatePresence>
        {activeShot && (
          <motion.div 
            initial={{ opacity: 0, y: -20 }}
            animate={{ opacity: 1, y: 0 }}
            className="absolute top-4 left-1/2 -translate-x-1/2 flex flex-col items-center gap-3 z-30"
          >
            ...
            {bestStreak >= 3 && !currentStreak && (
              <div className="text-xs text-nordic-forest/50 bg-white/20 backdrop-blur-md px-3 py-1 rounded-full">
