...
export const CameraSelector: React.FC<CameraSelectorProps> = ({ 
  ...
  return (
    <div className={`relative ${className}`}>
      ...
      {/* Dropdown Menu */}
      {isOpen && (
        <div className="absolute top-full right-0 mt-2 w-56 bg-white/80 backdrop-blur-xl rounded-2xl border border-white/80 overflow-hidden shadow-2xl z-50">
          {viewOptions.map((option) => (
            ...
            >
              <span className={`${currentView === option.id ? 'text-purple-600' : 'text-slate-400'}`}>
                {option.icon}
              </span>
